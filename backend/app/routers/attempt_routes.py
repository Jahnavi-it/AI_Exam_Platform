...
def get_result(
    ...
    # Milestone 3: students only see the result once an examiner has
    # reviewed every subjective answer and published it (objective-only
    # exams auto-publish on submit, so this never blocks them).
    # Examiners/admins can always see the in-progress state to do their grading.
    ...
    passed = None
    if exam.pass_marks is not None:
